...
function validateDocker() {
    ...
    (docker build . -t codiumteam/tdd-training-python) > /dev/null
    if [ $? -ne 0 ]; then
      echo "Error"
    ...
    (docker run --rm -v ${PWD}:/opt/project -w /opt/project codiumteam/tdd-training-python ls) > /dev/null
    if [ $? -ne 0 ]; then
      echo "Error"
...
validateDocker
validateMake
validateKata fizz-buzz "cd fizz-buzz" "make docker-test" "make docker-coverage"
validateKata roman-numerals "cd roman-numerals" "make docker-test" "make docker-coverage"
validateKata password-validator "cd password-validator" "make docker-test" "make docker-coverage"
validateKata user-registration "cd user-registration" "make docker-test" "make docker-coverage"
validateKata coffee-machine "cd coffee-machine" "make docker-test" "make docker-coverage"
